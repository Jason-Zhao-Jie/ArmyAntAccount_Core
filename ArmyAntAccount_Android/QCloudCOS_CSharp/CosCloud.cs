...
namespace QCloud.CosApi.Api
{
    ...
    class CosCloud
    {
        ...
        private string EncodeRemotePath(string remotePath)
        {
            ...
            foreach(var s in part)
            {
        	    if (s != "")
                {
                    ...
                    remotePath +=Java.Net.URLEncoder.Encode(s,"utf-8");
                }
            }
        ...
        {
            remotePath = StandardizationRemotePath(remotePath);
            var url = COSAPI_CGI_URL + appId + "/" + bucketName + EncodeRemotePath(remotePath) + Java.Net.URLEncoder.Encode(prefix,"utf-8");
            var data = new Dictionary<string, string>();
            data.Add("op", "list");
        ...
        public string SliceUploadFile(string bucketName, string remotePath, string localPath, int sliceSize = 512 * 1024)
        {
            ...
            var obj = new Org.Json.JSONObject(result);
            ...
            {
                var accessUrl = data.GetString("access_url");
                Console.WriteLine("命中秒传：" + accessUrl);
                return result;
            }
            else
            {
                var sessionId = data.GetString("session");
                sliceSize = data.GetInt("slice_size"); 
                var offset = data.GetInt("offset");
                var retryCount = 0;
                while(true)
                {
