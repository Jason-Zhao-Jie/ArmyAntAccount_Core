...
namespace ArmyAntAccount
{
	public partial class DataEdit : Form
	{
		...
		public DataEdit()
		{
			...
			{
				if(!typeBox.Items.Contains(Program.data.Data[i].type))
					typeBox.Items.Add(Program.data.Data[i].type);
				if(!ioPersonBox.Items.Contains(Program.data.Data[i].person))
					ioPersonBox.Items.Add(Program.data.Data[i].person);
				if(!tagBox.Items.Contains(Program.data.Data[i].tag))
					tagBox.Items.Add(Program.data.Data[i].tag);
			}
		...
		private void resetBtn_Click(object sender, EventArgs e)
		{
			...
			typeBox.SelectedText = data.type;
			if(data.change < 0)
				ioBox.SelectedIndex = 1;
			else
				ioBox.SelectedIndex = 0;
			ioNum.Value = Math.Abs(data.change);
			ioPersonBox.SelectedText = data.person;
			tagBox.SelectedText = data.tag;
